...
function initTree(){
	$.ajax( {
		url :'wechat/user/queryFwGroup.htm',
		type : 'post',
		dataType:'json',
	 ...
	 $(".nav li").each(function(){
			...
			if($(this).hasClass("active")){
				if(i==1){
					 ...
					 uploadFwNews(submitData);//上传图文消息
				}else if(i==3){
...
function doSendMessage(submitData){
	 $.ajax( {
 			url :'wechat/message/sendFwMessage.htm',
 			type : 'post',
...
function uploadFwNews(submitData){
	 $.ajax( {
			url :'wechat/media/uploadFwNews.htm',
			type : 'post',
			data:{"json":JSON.stringify(submitData)},
			dataType:'json',
			success : function(result) {
...
function uploadFwVideo(){
	var submitData={};
	    submitData["media_id"]=$("#media_video_id").val();
	    submitData["title"]="";
	    submitData["description"]="";
	 $.ajax( {
			url :'wechat/media/uploadFwVideo.htm',
			type : 'post',
			data:{"json":JSON.stringify(submitData)},
			dataType:'json',
			success : function(result) {
...
function initUploadBtn(K,buttonId,hiddenId,type){
	var uploadbutton = K.uploadbutton({
		button : K('#'+buttonId)[0],
		fieldName : 'imgFile',
		url : 'wechat/media/uploadFwMedia.htm?dir='+type,
