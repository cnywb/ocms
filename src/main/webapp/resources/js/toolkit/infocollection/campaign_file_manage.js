	...
	function initFileTree(){
		...
		$('#fileTree').fileTree({root: rootDir,script: 'toolkit/infocollection/campaign/file/list.htm'}, function(file,elementType,text) { 
	...
	function addFolder(){
		...
		$.ajax( {
			url :'toolkit/infocollection/campaign/file/folder/add.htm',
			type : 'post',
			dataType:'json',
			data:submitData,
	...
	function addFile(){
		...
		$.ajax( {
			url :'toolkit/infocollection/campaign/file/add.htm',
			type : 'post',
			dataType:'json',
			data:submitData,
			success : function(result) {
	...
	function doDelete(){
	   ...
	   $.blockUI({ message:'正在执行操作，请稍等...' });
		$.ajax( {
			url :'toolkit/infocollection/campaign/file/delete.htm',
			type : 'post',
			dataType:'json',
			data:submitData,
			success : function(result) {
	...
	function zipConfrim(zipSrcFile){
	   ...
	   function doZip(){
		    ...
		    $.blockUI({ message:'正在执行操作，请稍等...' });
			$.ajax( {
				url :'toolkit/infocollection/campaign/file/zip.htm',
				type : 'post',
				dataType:'json',
				data:submitData,
				success : function(result) {
	   ...
	   function doUnZip(){
		    ...
		    $.blockUI({ message:'正在执行操作，请稍等...' });
			$.ajax( {
				url :'toolkit/infocollection/campaign/file/unZip.htm',
				type : 'post',
				dataType:'json',
				data:submitData,
				success : function(result) {
	   ...
	   function appendParams(file){
		   ...
		   return file+'&moduleCode='+moduleCode;
	   }
	   function templateEdit(file){
		   file=encodeURI(file);
		   file=appendParams(file);
			window.parent.addTabs({id: "template_file_edit", title: "编辑模版文件", url: 'toolkit/infocollection/campaign/file/edit.htm?file='+file, close: true});
	   }
	   
	   function resourcesEdit(file){
		    file=encodeURI(file);
		    file=appendParams(file);
			window.parent.addTabs({id: "resources_file_edit", title: "编辑资源文件", url: 'toolkit/infocollection/campaign/file/edit.htm?file='+file, close: true});
	   }
